...
const VehicleNameInput = ({ initialName, onUpdate, className }: { initialName: string, onUpdate: (name: string) => void, className?: string }) => {
    const [name, setName] = useState(initialName);
    useEffect(() => setName(initialName), [initialName]);
    return (
        <input 
            className={`bg-transparent font-black tracking-tighter truncate focus:outline-none focus:border-indigo-500 border-b border-transparent hover:border-slate-600 transition-colors ${className || 'text-[10px] max-w-[100px] text-slate-200'}`}
            value={name}
...
const DEFAULT_VEHICLES: OBVehicle[] = [
    {
        id: 'OB-1',
        name: 'Unit 1 (Primary)',
        x: 400,
...
const ProximitySimulatorTab: React.FC = () => {
    console.log("ProximitySimulatorTab rendering");
    // Initial Setup: 2 OB Trucks in a compound
    const [vehicles, setVehicles] = useState<OBVehicle[]>(() => {
        const saved = localStorage.getItem('proximitySimulator_vehicles');
    ...
    const [mapZoom, setMapZoom] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_mapZoom');
        return saved ? JSON.parse(saved) : 0.8;
    }); // Visual zoom factor for UI
    const [panOffset, setPanOffset] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_panOffset');
        return saved ? JSON.parse(saved) : { x: 0, y: 0 };
    }); // Global map translation
    const [isRunning, setIsRunning] = useState(true);
    const [showLabels, setShowLabels] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_showLabels');
        return saved ? JSON.parse(saved) : true;
    });
    const [squelch, setSquelch] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_squelch');
        return saved ? JSON.parse(saved) : -95;
    });
    const [selectedRbw, setSelectedRbw] = useState<number>(() => {
        const saved = localStorage.getItem('proximitySimulator_selectedRbw');
        return saved ? JSON.parse(saved) : 0.025;
    }); // 25kHz
    ...
    const [tuningStep, setTuningStep] = useState<number>(() => {
        const saved = localStorage.getItem('proximitySimulator_tuningStep');
        return saved ? JSON.parse(saved) : 0.0125;
    }); 
    const [refLevel, setRefLevel] = useState<number>(() => {
        const saved = localStorage.getItem('proximitySimulator_refLevel');
        return saved ? JSON.parse(saved) : 0;
    });
    const [showFloatingMonitor, setShowFloatingMonitor] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_showFloatingMonitor');
        return saved ? JSON.parse(saved) : false;
    });

    const [isAutoView, setIsAutoView] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_isAutoView');
        return saved ? JSON.parse(saved) : true;
    });
    const [centerFreq, setCenterFreq] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_centerFreq');
        return saved ? JSON.parse(saved) : 457.0;
    });
    const [spanMhz, setSpanMhz] = useState(() => {
        const saved = localStorage.getItem('proximitySimulator_spanMhz');
    ...
    useEffect(() => {
        localStorage.setItem('proximitySimulator_vehicles', JSON.stringify(vehicles));
        localStorage.setItem('proximitySimulator_mapZoom', JSON.stringify(mapZoom));
        localStorage.setItem('proximitySimulator_panOffset', JSON.stringify(panOffset));
        localStorage.setItem('proximitySimulator_showLabels', JSON.stringify(showLabels));
        localStorage.setItem('proximitySimulator_squelch', JSON.stringify(squelch));
        localStorage.setItem('proximitySimulator_selectedRbw', JSON.stringify(selectedRbw));
        localStorage.setItem('proximitySimulator_tuningStep', JSON.stringify(tuningStep));
        localStorage.setItem('proximitySimulator_refLevel', JSON.stringify(refLevel));
        localStorage.setItem('proximitySimulator_showFloatingMonitor', JSON.stringify(showFloatingMonitor));
        localStorage.setItem('proximitySimulator_isAutoView', JSON.stringify(isAutoView));
        localStorage.setItem('proximitySimulator_centerFreq', JSON.stringify(centerFreq));
        localStorage.setItem('proximitySimulator_spanMhz', JSON.stringify(spanMhz));
    }, [vehicles, mapZoom, panOffset, showLabels, squelch, selectedRbw, tuningStep, refLevel, showFloatingMonitor, isAutoView, centerFreq, spanMhz]);
    ...
    const analyzerPointers = useRef<Map<number, { x: number, y: number }>>(new Map());

    const handleAnalyzerPointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
        e.currentTarget.setPointerCapture(e.pointerId);
        analyzerPointers.current.set(e.pointerId, { x: e.clientX, y: e.clientY });
    };

    const handleAnalyzerPointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
        const rect = e.currentTarget.getBoundingClientRect();
        const mouseX = e.clientX - rect.left;
        const mouseY = e.clientY - rect.top;

        if (analyzerPointers.current.has(e.pointerId)) {
            const start = analyzerPointers.current.get(e.pointerId)!;
            const dx = e.clientX - start.x;
            
            if (dimensions.width > 0) {
                const freqChange = (dx / dimensions.width) * spanMhz;
                setCenterFreq(prev => prev - freqChange);
                setIsAutoView(false);
            }
        ...
        // Tooltip logic
        const canvasX = mouseX * 2;
        const canvasY = mouseY * 2;
        const pad = { top: 75, right: 30, bottom: 100, left: 110 };
        const chartW = dimensions.width * 2 - pad.left - pad.right;
        const chartH = dimensions.height * 2 - pad.top - pad.bottom;
        
        if (canvasX >= pad.left && canvasX <= pad.left + chartW && canvasY >= pad.top && canvasY <= pad.top + chartH) {
            const minF = centerFreq - spanMhz / 2;
            const freq = minF + ((canvasX - pad.left) / chartW) * spanMhz;
            
            const peaks: any[] = [];
            const thresholdFreq = spanMhz * 0.015; // 1.5% of span threshold

            simData.carriers.forEach(c => {
                const d = Math.abs(c.freq - freq);
                if (d < thresholdFreq) {
                    peaks.push({ freq: c.freq, power: c.pRx, type: 'Fundamental', label: c.id });
                }
            });

            simData.imds.forEach(im => {
                const d = Math.abs(im.val - freq);
                if (d < thresholdFreq) {
                    peaks.push({ 
                        freq: im.val, 
                        power: im.power, 
                        type: im.type, 
                        label: im.formula, 
                        sourceFreqs: im.sourceFreqs,
                        minIsolation: im.minIsolation
                    });
                }
            });
            ...
            if (peaks.length > 0) {
                ...
                setHoveredPeak({ x: mouseX, y: mouseY, peaks });
            } else {
                setHoveredPeak(null);
            }
        } else {
    ...
    const handleAnalyzerPointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
        e.currentTarget.releasePointerCapture(e.pointerId);
        analyzerPointers.current.delete(e.pointerId);
    };
            ...
            for (let j = 0; j < flattenedCarriers.length; j++) {
                ...
                if (pImdRx2t > -115) {
                    imds.push({
                        ...
                        sourceFreqs: [s1.freq, s2.freq],
                        minIsolation: isolation2t
                    });
                }
                    ...
                    for (let k = j + 1; k < flattenedCarriers.length; k++) {
                        ...
                        if (pImdRx3t > -115) {
                            imds.push({
                                ...
                                sourceFreqs: [s1.freq, s2.freq, s3.freq],
                                minIsolation: Math.min(iso12, iso13, iso23)
                            });
                        }
                    }
                }
    ...
    const uniqueAntennas = useMemo(() => {
        console.log("uniqueAntennas recomputing, vehicles:", vehicles);
        const ants: { id: string, x: number, y: number, type: string, vehicleName: string, vehicleId: string }[] = [];
        vehicles.forEach(v => {
            const rad = (v.rotation * Math.PI) / 180;
            v.antennas.forEach(a => {
                const absX = v.x + (a.relX * Math.cos(rad) - a.relY * Math.sin(rad));
                const absY = v.y + (a.relX * Math.sin(rad) + a.relY * Math.cos(rad));
                ants.push({ id: a.id, x: absX, y: absY, type: a.type, vehicleName: v.name, vehicleId: v.id });
            });
        });
        return ants;
    }, [vehicles]);

    const updateVehicleName = (vId: string, name: string) => {
        console.log("Updating vehicle name:", vId, name);
    ...
    const centerUnits = () => {
        ...
        setPanOffset({ 
            x: container.offsetWidth / 2 - (600 * mapZoom), 
            y: container.offsetHeight / 2 - (300 * mapZoom) 
        });
    };

    // Center the view on initial mount
    useEffect(() => {
        const savedPanOffset = localStorage.getItem('proximitySimulator_panOffset');
        if (!savedPanOffset && mapRef.current && mapRef.current.parentElement) {
            const container = mapRef.current.parentElement;
            setPanOffset({ 
                x: container.offsetWidth / 2 - (600 * 0.8), 
                y: container.offsetHeight / 2 - (300 * 0.8) 
            });
        }
    }, []);
            ...
            <div className="relative w-full h-full">
                {vehicles.map(v => (
                    ...
                    >
                        ...
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 whitespace-nowrap bg-black/80 px-2 py-1 rounded text-[10px] sm:text-xs font-black text-white uppercase tracking-tighter border border-white/10 shadow-xl z-20">
                            {v.name}
                        </div>
                    </div>
                ))}
    ...
    return (
        <div className="flex flex-col gap-6 relative">
            ...
            {/* TOP ROW: MAP AND ANALYZER */}
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                {/* COMPOUND MAP */}
                <Card className="!p-0 overflow-hidden relative border-2 border-indigo-500/20 shadow-2xl rounded-2xl bg-[#020617] flex flex-col h-[600px] xl:h-[700px]">
                    <div className="bg-slate-900/90 p-4 border-b border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-20 shrink-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button onClick={centerUnits} className="px-4 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-lg text-[10px] font-black uppercase tracking-widest text-white shadow-lg transition-all" title="Shift all vehicles to center of logically visible area">🎯 Center</button>
                            <button onClick={() => { 
                                setMapZoom(0.8); 
                            ...
                            }} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-lg text-[10px] font-black uppercase tracking-widest text-white shadow-lg transition-all">Reset View</button>
                            <button onClick={addTruck} className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-[10px] font-black uppercase tracking-widest text-white shadow-lg transition-all">+ Add</button>
                        </div>
                    </div>

                    <div className="relative w-full flex-1 overflow-hidden bg-black/40 touch-none">
                    <div 
                        ref={mapRef} 
                        ...
                        {vehicles.map(v => (
                            ...
                            >
                                <div className="relative w-full h-full group">
                                    <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 border border-white/10 px-3 py-1 rounded text-sm sm:text-base font-black text-white uppercase tracking-widest whitespace-nowrap opacity-90 group-hover:opacity-100 transition-opacity z-50 shadow-lg pointer-events-none" style={{ transform: `rotate(${-v.rotation}deg)` }}>
                                        {v.name}
                                    </div>
                                    <div 
                ...
                <Card className="!p-0 border-indigo-500/20 shadow-2xl overflow-hidden rounded-2xl flex flex-col h-[600px] xl:h-[700px]">
                    <div className="bg-slate-950 p-5 border-b border-white/10 flex flex-col justify-between items-start gap-4 shrink-0">
                        <div className="flex items-center gap-4">
                            <div className={`w-3 h-3 rounded-full bg-cyan-500 animate-pulse`}></div>
                            <h3 className="text-sm font-black uppercase tracking-[0.5em] text-indigo-300">Interaction Analyzer</h3>
                        </div>
                        
                        <div className="flex flex-wrap items-center gap-4 bg-black/40 p-3 rounded-2xl border border-white/5 shadow-inner w-full">
                            ...
                            <div className="flex flex-col pr-4 border-r border-white/10">
                                <span className="text-[8px] text-slate-500 uppercase font-black tracking-widest">Step Size</span>
                                <div className="flex bg-slate-800 rounded-lg p-0.5 mt-1.5 border border-white/5">
                                    {[0.0125, 0.1, 1.0, 10.0].map(val => (
                                        <button 
                                            key={val} 
                                            onClick={() => setTuningStep(val)} 
                                            className={`px-2 py-1.5 rounded text-[9px] font-black transition-all ${tuningStep === val ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                                        >
                                            {val < 1 ? `${val * 1000}k` : `${val}M`}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col pr-4 border-r border-white/10">
                                ...
                                <div className="flex items-center gap-1 mt-1.5">
                                    <button onClick={() => { setIsAutoView(false); setCenterFreq(f => f - tuningStep); }} className="w-8 h-8 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded-lg text-white font-black border border-white/5 transition-colors shadow-lg">&lt;</button>
                                    <div className="flex items-center bg-slate-900 border border-indigo-500/20 rounded-lg px-2 py-1.5 shadow-inner">
                                        <input 
                                            type="number" 
                                            step={tuningStep} 
                                            value={centerFreq} 
                    ...
                    <div className="relative group flex-1">
                        <canvas 
                            ...
                            onPointerDown={handleAnalyzerPointerDown}
                            onPointerMove={handleAnalyzerPointerMove}
                            onPointerUp={handleAnalyzerPointerUp}
                            onPointerLeave={(e) => { handleAnalyzerPointerUp(e); setHoveredPeak(null); }}
                        />
                        {hoveredPeak && (
                            <div 
                                className="absolute pointer-events-none bg-slate-900/95 border border-indigo-500/50 rounded-lg p-2.5 shadow-2xl z-50 backdrop-blur-sm min-w-[140px]"
                                style={{ 
                                    left: `${hoveredPeak.x}px`, 
                                    top: `${hoveredPeak.y - 10}px`,
                                    transform: 'translate(-50%, -100%)'
                                }}
                            >
                                <div className="flex flex-col gap-3">
                                    {hoveredPeak.peaks.map((peak, idx) => (
                                        <div key={idx} className="flex flex-col gap-1">
                                            {idx > 0 && <div className="h-px bg-white/20 my-1 shadow-[0_0_5px_rgba(255,255,255,0.1)]" />}
                                            <div className="flex justify-between items-center gap-4">
                                                <span className={`text-[10px] font-black uppercase tracking-wider ${peak.type === 'Fundamental' ? 'text-cyan-400' : 'text-indigo-400'}`}>
                                                    {peak.type}
                                                </span>
                                                <span className="text-[9px] font-mono text-slate-500">{peak.label}</span>
                                            </div>
                                            <div className="h-px bg-white/10 my-0.5" />
                                            <div className="flex justify-between items-baseline">
                                                <span className="text-[11px] font-mono text-cyan-300 font-bold">{peak.freq.toFixed(4)}</span>
                                                <span className="text-[8px] text-slate-500 font-bold ml-1 uppercase">MHz</span>
                                            </div>
                                            <div className="flex justify-between items-baseline">
                                                <span className="text-[11px] font-mono text-emerald-400 font-bold">{peak.power.toFixed(1)}</span>
                                                <span className="text-[8px] text-slate-500 font-bold ml-1 uppercase">dBm</span>
                                            </div>
                                            ...
                                            {peak.sourceFreqs && (
                                                <>
                                                    ...
                                                    <div className="flex flex-col gap-0.5">
                                                        <span className="text-[8px] text-slate-500 uppercase font-bold tracking-tighter">Sources:</span>
                                                        <div className="flex flex-wrap gap-x-2 gap-y-0.5">
                                                            {peak.sourceFreqs.map((f, i) => (
                                                                <span key={i} className="text-[9px] font-mono text-indigo-300">{f.toFixed(3)}</span>
                                                            ))}
                                                        </div>
                                                    </div>
                                                </>
                                            ...
                                            {peak.minIsolation !== undefined && (
                                                <div className="mt-1 flex items-center gap-1.5">
                                                    <div className={`w-1.5 h-1.5 rounded-full ${peak.minIsolation < 40 ? 'bg-red-500 animate-pulse' : peak.minIsolation < 70 ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                                                    <span className={`text-[9px] font-black uppercase tracking-tighter ${peak.minIsolation < 40 ? 'text-red-400' : peak.minIsolation < 70 ? 'text-amber-400' : 'text-emerald-400'}`}>
                                                        {peak.minIsolation < 40 ? 'Critical Proximity' : peak.minIsolation < 70 ? 'High Proximity' : 'Low Proximity'}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="absolute bottom-[-6px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[6px] border-t-indigo-500/50" />
                            </div>
                        )}
                    </div>
                </Card>
            ...
            <div className="bg-slate-950 border border-indigo-500/20 p-4 rounded-2xl shadow-xl mt-6">
                <div className="flex justify-between items-center mb-4 text-center">
                    <h4 className="text-sm font-black text-indigo-400 uppercase tracking-[0.3em] w-full">Spatial Proximity Ledger</h4>
                </div>
                        ...
                        <thead>
                            <tr className="bg-slate-900">
                                ...
                                {uniqueAntennas.map(a => (
                                    <th key={a.id} className="p-2 border border-slate-800 text-slate-300 font-bold uppercase tracking-tighter min-w-[120px]">
                                        <VehicleNameInput 
                                            initialName={a.vehicleName}
                                            onUpdate={(name) => updateVehicleName(a.vehicleId, name)}
                                            className="text-base max-w-[120px] text-center text-slate-300"
                                        />
                                    </th>
                                ))}
                            </tr>
                            ...
                            {uniqueAntennas.map((a1, i) => (
                                <tr key={a1.id} className="hover:bg-indigo-500/5 transition-colors">
                                    <th className="p-2 border border-slate-800 bg-slate-900 text-slate-400 font-bold text-left uppercase whitespace-nowrap">
                                        <VehicleNameInput 
                                            initialName={a1.vehicleName}
                                            onUpdate={(name) => updateVehicleName(a1.vehicleId, name)}
                                            className="text-base max-w-[160px] text-slate-400"
                                        />
                                    </th>
                                    {uniqueAntennas.map((a2, j) => {
                                        ...
                                        return (
                                            <td 
                                                key={a2.id} 
                                                className={`p-2 border border-slate-800 font-mono text-base transition-all ${
                                                    ...
                                                    <div key={tx.id} className="grid grid-cols-[1fr,65px,auto] gap-2 items-center bg-black/40 p-1.5 rounded-lg border border-white/5 hover:border-white/10 transition-colors">
                                                        <div className="flex items-center gap-2">
                                                            <button 
                                                                ...
                                                                className={`relative inline-flex h-3.5 w-7 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                                                                    tx.active ? (a.type === 'Colinear' ? 'bg-cyan-600 shadow-[0_0_8px_rgba(6,182,212,0.4)]' : 'bg-amber-600 shadow-[0_0_8px_rgba(245,158,11,0.4)]') : 'bg-slate-800'
                                                                }`}
                                                                title={tx.active ? "Deactivate Frequency" : "Activate Frequency"}
                                                            >
                                                                <span
                                                                    aria-hidden="true"
                                                                    className={`pointer-events-none inline-block h-2.5 w-2.5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                                                                        tx.active ? 'translate-x-3.5' : 'translate-x-0'
                                                                    }`}
                                                                />
                                                            </button>
...
export default ProximitySimulatorTab;