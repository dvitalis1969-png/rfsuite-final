            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    ...
                    <div className="space-y-2.5 max-h-[480px] overflow-y-auto pr-3 custom-scrollbar">
                        {frequencies.map((f) => (
                            <div key={f.id} className="grid grid-cols-[24px_minmax(80px,100px)_minmax(120px,220px)_80px_1fr_36px_36px] gap-2 items-center group bg-slate-950/20 p-1.5 rounded-xl border border-transparent hover:border-white/5 transition-all w-full">
                                ...
                                <FrequencyValueInput
                                    ...
                                    className="bg-slate-900 border border-white/5 rounded-lg p-2 text-indigo-400 text-xs font-bold font-mono focus:border-indigo-500 outline-none text-center shadow-inner w-full"
                                />
                                <input
                                    type="text"
                                    placeholder="Label"
                                    ...
                                    className="bg-slate-900 border border-white/5 rounded-lg p-2 text-slate-300 text-xs font-bold focus:border-indigo-500 outline-none shadow-inner w-full"
                                />
                                <select value={f.type || 'generic'} onChange={e => handleFrequencyChange(f.id, 'type', e.target.value)} className="bg-slate-800 border border-white/5 rounded-lg p-2 text-slate-400 text-[9px] font-black uppercase tracking-tighter w-full">
                                ...
                                <button onClick={() => handleLockToggle(f.id)} className={`p-2 rounded-xl transition-all border flex items-center justify-center ${f.locked ? 'bg-amber-500/10 border-amber-500/40 text-amber-500' : 'bg-slate-900 border-white/10 text-slate-600 hover:text-white'}`}>
                                    {f.locked ? '🔒' : '🔓'}
                                </button>
                                <button onClick={() => removeFrequency(f.id)} className="text-rose-500/30 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all p-1 flex items-center justify-center">
